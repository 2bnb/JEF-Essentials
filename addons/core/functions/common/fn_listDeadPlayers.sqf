/* ----------------------------------------------------------------------------
Function: bnb_es_core_fnc_listDeadPlayers
...
Examples:
	[east] call bnb_es_core_fnc_listDeadPlayers;
...
private _deadPlayers = [] call bnb_es_core_fnc_getDeadPlayers;

if (_side isEqualTo "") then {
	_deadPlayers = _deadPlayers call bnb_es_core_fnc_arrayFlatten;
} else {
...
[format["Listing dead players: %1", _deadPlayers], "core\functions\common\fn_listDeadPlayers.sqf"] call bnb_es_core_fnc_log;
private _message = "Dead players:\n";
private _deathDuration = 0;
private _diedAt = 0;
{
	_diedAt = _x getVariable ["bnb_es_diedAt", serverTime];
	_deathDuration = [(serverTime - _diedAt), "M:SS"] call CBA_fnc_formatElapsedTime;
	[format["Dead: %1 in %2 at %3 out of %4 (%5)", name _x, side group _x, _diedAt, serverTime, _deathDuration], "core\functions\common\fn_listDeadPlayers.sqf"] call bnb_es_core_fnc_log;
